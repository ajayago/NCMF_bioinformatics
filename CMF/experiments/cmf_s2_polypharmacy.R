...
#data_folder <- '../HNE/Data/Polypharmacy/CMF/'
data_folder <- '../../datasets/CMF/Polypharmacy/sampled2/'
test_link_file <- 'sampled2_link.dat.test' # TODO
emb_file <- 'cmf_s2_polypharmacy_emb.dat' # TODO
...
D <- c(645, 837) # node count for Polypharmacy dataset
inds <- matrix(0, nrow=3, ncol=2) # entity
inds[1,] <- c(1, 1)
inds[2,] <- c(1, 2)
inds[3,] <- c(2, 2) # only three matrices exist between the drug-gene datasets
#inds[4,] <- c(3, 1)
#inds[5,] <- c(3, 2)
#inds[6,] <- c(3, 3)
#inds[7,] <- c(3, 4)
...
X[[1]][test_link_df[, 'idx_from'], test_link_df[, 'idx_to']] <- NA # drug-drug target hidden test link
...
likelihood <- c("bernoulli", "bernoulli", "bernoulli") # all three matrices have zeros and ones only => bernoulli
opts <- getCMFopts()
