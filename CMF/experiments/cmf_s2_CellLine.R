...
data_folder <- '../../datasets/CMF/CellLine/sampled2/'
test_link_file <- 'sampled2_link.dat.test' # TODO
emb_file <- 'cmf_s2_CellLine_emb.dat' # TODO
...
D <- c(692, 638, 396) # node count for Polypharmacy dataset
inds <- matrix(0, nrow=4, ncol=2) # entity
inds[1,] <- c(1, 2)
inds[2,] <- c(1, 2)
inds[3,] <- c(1, 2)
inds[4,] <- c(1, 3)
...
print("Merged")
X[[4]][test_link_df[, 'idx_from'], test_link_df[, 'idx_to']] <- NA # drug-drug target hidden test link
...
likelihood <- c("bernoulli", "gaussian", "gaussian", "bernoulli") # all three matrices have zeros and ones only => bernoulli
opts <- getCMFopts()
