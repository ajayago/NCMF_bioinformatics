...
def load_data(args, val_ratio=0.1, test_ratio=0.2):
    ...
    test_data_df = pd.read_csv("../datasets/NCMF/PolyP3/sampled3_link.dat.test", sep="\t", header=None)
    test_data_df.columns = ["left", "right", "value"]
    test_data = test_data_df.sort_values(by="value", ascending=False).values
...
#                        shape=(unique_entity, unique_entity), dtype=np.float32)
    adj = sp.coo_matrix((np.ones(train_positive.shape[0]), (train_positive[:, 0], train_positive[:, 1])),shape=(1482, 1482), dtype=np.float32)
    ...
    np.save(f"{args.in_file[5:8]}_train.npy", train_data)
    np.save(f"{args.in_file[5:8]}_test.npy", test_data)
    np.save(f"{args.in_file[5:8]}_val.npy", val_data)
