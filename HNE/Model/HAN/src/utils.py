...
def load_data_unsupervised(args, node, edge, config, meta):
    ...
    # TODO: Custom targets for Pubmed link_type 1
    target, positive_type = int(lines[0][:-1]), int(lines[1][:-1])
    print(f"Positive type = {positive_type}")
    # targets = [0, 1]
    # positive_type = 1
    ...
    for each in lines[2].split('\t'):
        ...
        # if ltype==positive_type and start==targets[0] and end==targets[1]:
            positive_same = True
