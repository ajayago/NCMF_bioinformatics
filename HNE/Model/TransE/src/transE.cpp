...
void init() {
	...
	fin = fopen(relationPath.c_str(), "r");
...
void write_para(FILE* writef) {
    fprintf(writef, "size=%d, ", dimension);
    fprintf(writef, "epochs=%d, ", trainTimes);
    fprintf(writef, "nbatches=%d, ", nbatches);
    fprintf(writef, "alpha=%f, ", alpha);
    fprintf(writef, "margin=%f", margin);
    fprintf(writef, "\n");
}
...
// 			out_binary(); 
		...
		FILE* f2 = fopen(outPath.c_str(), "w");
		write_para(f2);
		for (INT i = 0; i < entityTotal; i++) {
			fprintf(f2, "%d\t", i);
			INT last = i * dimension;
			for (INT ii = 0; ii < dimension; ii++)
				fprintf(f2, "%.6f ", entityVec[last + ii]);
			fprintf(f2,"\n");
		}
// 		for (INT i=0; i < relationTotal; i++) {
// 			fprintf(f2, "%d\t", i+entityTotal);        
// 			INT last = i * dimension;
// 			for (INT ii = 0; ii < dimension; ii++)
...
void setparameters(int argc, char **argv) {
	...
	if ((i = ArgPos((char *)"-entity", argc, argv)) > 0) entityPath = argv[i + 1];
	if ((i = ArgPos((char *)"-relation", argc, argv)) > 0) relationPath = argv[i + 1];
	if ((i = ArgPos((char *)"-triplet", argc, argv)) > 0) tripletPath = argv[i + 1];
