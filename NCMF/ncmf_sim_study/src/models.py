...
class DCMF(nn.Module):
    def __init__(self, graph, meta, entity_dimensions, autoencoder_config, reconstructor_config, fusion_config, matrix_types):
    ...
    def __init_reconstructors(self, in_feat_dict):
        ...
        for id, _ in self.meta.items():
            if id in self.matrix_types["binary"]:
                self.reconstructors[id] = NMF_hadamard(in_features=2 * self.aec_config['k'], activation=self.rec_config['activation_function'])
            elif id in self.matrix_types["real"]:
                self.reconstructors[id] = NMF_hadamard(in_features=2 * self.aec_config['k'], activation=self.rec_config['activation_function'], output_activation = self.rec_config['output_activation_function_real'])
...
class NMF_hadamard(nn.Module):
    def __init__(self, in_features: int, activation: str, output_activation: str = None) -> None:
        super(NMF_hadamard, self).__init__()
        self.activations_available = nn.ModuleDict({
            'relu': nn.ReLU(),
            'lrelu': nn.LeakyReLU(),
            'selu': nn.SELU(),
        ...
        self.actf = self.activations_available[activation]
        print(f"NMF hadamard {in_features // 2}")
        self.fc1 = nn.Linear(in_features // 2, 150)
        self.fc2 = nn.Linear(150, 100)
        #self.fc3 = nn.Linear(50, 100)
        #self.fc4 = nn.Linear(125, 100)
        # self.d1 = nn.Dropout(0.2)
        self.fc31 = nn.Linear(100, 1)
        self.fc32 = nn.Linear(100, 1)
        self.fc33 = nn.Linear(100, 1)
        if output_activation:
            self.output_activation = self.activations_available[output_activation]
        else:
    ...
    def forward(self, row_emb: Tensor, col_emb: Tensor) -> Tensor:
        ...
        #h4 = self.actf(self.fc4(h3))
        # h2 = self.d1(h2)

        if not self.output_activation: # binary case
            M_bar = torch.clamp(torch.exp(self.fc31(h2)), 1e-5, 1e6)
            # M_bar = self.actf(self.fc31(h3))
            Theta = torch.clamp(torch.exp(self.fc32(h2)), 1e-5, 1e6)
            Pi = torch.sigmoid(self.fc33(h2))
        else: # real valued case
            M_bar = self.output_activation(self.fc31(h2))
            Theta = torch.clamp(torch.exp(self.fc32(h2)), 1e-5, 1e6)
            Pi = torch.sigmoid(self.fc33(h2))
        return M_bar, Theta, Pi
