...
class DCMF(nn.Module):
    ...
    def vae_forward(self, ids, matrices):
        ...
        for (xid, dim), x in zip(ids, matrices):
            entity = {}
            entity['hidden'], entity['M_bar'], entity['Theta'], entity['Pi'], entity['mu'], entity['logvar'] = self.autoencoders[xid][dim](
                x, self.dummy)
            entities += [entity]
        return entities
    ...
    def rec_forward(self, id, row_emb, col_emb):
        rec = {}
        rec['h2'], rec['M_bar'], rec['Theta'], rec['Pi'] = self.reconstructors[id](
            row_emb, col_emb)
        return rec
...
class VariationalDecoder(AutoEncoderBase):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return self.output_layers['M_bar'](hidden), M_bar, Theta, Pi
...
class VariationalAutoencoder(nn.Module):
    ...
    def forward(self, x: Tensor, d: Tensor) -> Tuple[Tensor, float, float]:
        ...
        hidden, M_bar, Theta, Pi = self.decoder(z)
        return hidden, M_bar, Theta, Pi, mu, logvar
...
class NMF(nn.Module):
    ...
    def forward(self, row_emb: Tensor, col_emb: Tensor) -> Tensor:
        ...
        return self.fc31(h2), M_bar, Theta, Pi
