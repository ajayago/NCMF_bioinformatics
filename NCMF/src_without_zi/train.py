...
from src_without_zi.loss import *
...
#                        print(b.shape)
                ...
                scaler.update()
                scheduler.step()
                #print(f"Row loss = {row_loss.item()}")
                #print(f"Col loss = {col_loss.item()}")
        ...
        for i, (row, col, val) in enumerate(dataloader):
            ...
            with amp.autocast():
                ...
                zinb_loss = updated_loss_func(
                    XP_block['h2'], X_block)
                rmse_loss = rmse_loss_func(XP_block['h2'], X_block, None)

                total_loss += zinb_loss.item()
                total_rmse += rmse_loss.item()
    #            total_loss = total_rmse
    return total_loss, total_rmse
