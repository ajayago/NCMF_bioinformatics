...
def main():
    ...
    data_folder = f'../../datasets/CMF/CellLine/sampled{sample_id}/'
    test_links = f'sampled{sample_id}_link.dat.test'
    emb_file = f'dfmf_s{sample_id}_emb_CellLine.dat'
    k = 50
    ...
    relations = [
        fusion.Relation(X0, e0, e1),
        fusion.Relation(X1, e0, e1),
        fusion.Relation(X2, e0, e1),
        fusion.Relation(X3, e0, e2)
    ]
    ...
    print('Saving...')
    embs = []
    entities = [e0, e1, e2] # must be in chronological order
